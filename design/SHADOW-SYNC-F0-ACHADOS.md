...
|---|---|---|---|
| empresa | ~~TABEMPRESAS.NOME~~ **`TABFILIAL.NOME`** | 98% → ~100% | **CORRIGIDO (2026-07-10, diff plano×realidade):** o 1º segmento é o NOME DA FILIAL — filiais da mesma empresa têm pastas com nomes DIFERENTES (caso JOAO BATISTA emp 369: fil 1 "FAZENDA CONJUNTO LINDOIA", fil 2 "JOAO BATISTA ... - FAZENDA CONJUNTO LINDOIA", ambas batendo com TABFILIAL.NOME). Os "2% de nome editado" eram na verdade FONTE ERRADA (TABEMPRESAS). Syncer/check-path corrigidos: TABFILIAL.NOME com fallback TABEMPRESAS.NOME. ~~Regra `&`→`e` confirmada em pasta real (TRINDADE)~~ **⚠️ REVISTO NA F1 (2026-07-14, commit `53aaa5b`): a regra `&`→`e` e o corte de ponto final estavam ERRADOS.** O `--check-plans` achou 735 divergências e o experimento sobre TABFILIAL×URL mostrou que 13/13 filiais com `&` MANTÊM o `&` na pasta real e as com ponto final MANTÊM o ponto — o DownloadXML usa o NOME verbatim. O `SanitizeSegment` agora só remove reservado-NTFS + controle. Ver F1 no SHADOW-SYNC.md §12.8 |
...
| direção | `DirectionFromCNPJs` | ~~**100%**~~ **≈98%** (NFe/NFCe) | **⚠️ REVISTO NA F1 (commit `2d54fc8`):** o `--check-path` derivava da linha real e não pegou as DEVOLUÇÕES — 385 notas emitidas pela empresa (`tpNF=0`) que o DownloadXML arquiva em ENTRADA e o `DirectionFromCNPJs` (só-CNPJ) marcava SAIDA. O syncer agora lê o `<tpNF>`: emitente + `tpNF=0` → ENTRADA. Ver §12.7 |
